...
The desktop app runs on **Windows**, **macOS**, and **Linux** and supports multiple Viya server connections with per-connection authentication.
...
You can just download the [./dist.zip] and unzip that onto your webserver. If it is under the same URL origin as the SAS Viya environment you do not have to do anything else - otherwise edit the config.js file and enter the URL of your SAS Viya environment for the SAS_VIYA_URL value.
